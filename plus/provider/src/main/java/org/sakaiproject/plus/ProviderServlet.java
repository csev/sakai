...
public class ProviderServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// Support the new LtiContextLaunchRequest message if we ever see it
		if ( LaunchJWT.MESSAGE_TYPE_LTI_CONTEXT.equals(launchJWT.message_type) ) {
			...
			tool_id = pieces[pieces.length-1];
		}
	...
	protected void handleDynamicRegistration(HttpServletRequest request, HttpServletResponse response, String tenant_guid) throws ServletException, IOException {
		...
		// No tools - kind of weird - but punt and install deep.link, let them fix it later
		...
		// but LMS's with better placement options, have options
			...
			lm.type = LaunchJWT.MESSAGE_TYPE_LAUNCH;
			lm.label = rb.getString("plus.deeplink.sakai.plus");
			lm.target_link_uri = plusService.getPlusServletPath() + "/" + SAKAI_SITE_LAUNCH;
			ltitc.messages.add(lm);
