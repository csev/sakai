	...
	public void syncSiteMemberships(String contextGuid, Site site) throws LTIException {
		...
		try {
			...
			while (true) {
				...
				cLog = new ContextLog();
				cLog.setType(ContextLog.LOG_TYPE.NRPS_MEMBER);
				cLog.setContext(context);
				cLog.setSuccess(Boolean.FALSE);
				cLog.setAction("processing member="+member.email+" user_id="+member.user_id+" count="+count);
				dbs = new StringBuffer();
				...
				if ( subject == null ) {
					...
					cLog.setStatus("Failed createOrUpdateSubject subject="+member.user_id);
					contextLogRepository.save(cLog);
					continue;
				}
				...
				if ( user == null ) {
					...
					cLog.setStatus("Failed findOrCreateUser subject="+member.user_id);
					contextLogRepository.save(cLog);
					continue;
				}
			...
			cLog = new ContextLog();
			cLog.setType(ContextLog.LOG_TYPE.NRPS_ERROR);
			cLog.setContext(context);
			cLog.setSuccess(Boolean.FALSE);
		...
		cLog.setStatus("Completed syncSiteMemberships at="+Instant.now());
		cLog.setSuccess(Boolean.TRUE);
		contextLogRepository.save(cLog);
	}
